...
const FeedbackToast = ({ feedback, score, t }) => (
    ...
    if (phase === 'setup') return (
        <div className="iv-page" dir={dir}>
            <div className="iv-header">
                <button className="vi-back-btn" onClick={onBack} title={t('interviewBackToMode')}>
                    <ArrowLeft size={18} />
                </button>
                ...
                <div>
                    <h1 className="iv-title">{t('interviewVoiceMode')}</h1>
                    <p className="iv-subtitle">{t('interviewVoiceSubtitle')}</p>
                </div>
            </div>
            ...
            {!voice.isSupported && (
                <div className="iv-error">
                    ...
                    <span>{t('interviewBrowserNotSupported')}</span>
                </div>
            )}
            ...
            <div className="iv-page" dir={dir}>
                ...
                <div className="iv-results-grid">
                    {results.strengths?.length > 0 && (
                        <div className="iv-card">
                            <div className="iv-card-header">
                                <Star size={16} className="iv-icon-green" /><h3>{t('interviewStrengths')}</h3>
                            </div>
                    ...
                    {results.improvements?.length > 0 && (
                        <div className="iv-card">
                            <div className="iv-card-header">
                                <TrendingUp size={16} className="iv-icon-blue" /><h3>{t('interviewImprovements')}</h3>
                            </div>
                ...
                <div className="iv-actions">
                    <button className="iv-btn iv-btn-primary" onClick={handleReset}>
                        <RotateCcw size={16} /><span>{t('startNewInterview')}</span>
                    </button>
                </div>
            </div>
        );
        ...
        <div className="iv-page" dir={dir}>
            ...
            <div className="iv-header">
                ...
                <button
                    ...
                    title={voiceEnabled ? t('interviewDisableAiVoice') : t('interviewEnableAiVoice')}
                >
                    {voiceEnabled ? <Volume2 size={15} /> : <VolumeX size={15} />}
                    <span>{voiceEnabled ? t('interviewAiVoiceOn') : t('interviewAiVoiceOff')}</span>
                </button>
            </div>

            {/* Progress bar */}
            <div className="iv-progress-bar-wrap">
                <div className="iv-progress-label">{t('interviewQuestion')} {qNum} {t('interviewOf')} {qTotal}</div>
            ...
            <div className="iv-card vi-question-card">
                <div className="vi-question-header">
                    ...
                    <span className="iv-bubble-meta">{t('interviewQuestion')} {qNum} / {qTotal}</span>

                    {/* Stop-speech button shown only while speaking */}
                    {voice.isSpeaking && (
                        <button className="vi-stop-speech-btn" onClick={voice.stopSpeaking} title={t('interviewStopSpeaking')}>
                            <VolumeX size={13} /> {t('interviewStopSpeaking')}
                        </button>
                    )}
                </div>
            ...
            {lastFeedback && (
                <FeedbackToast feedback={lastFeedback.text} score={lastFeedback.score} t={t} />
            )}
            ...
            <div className="vi-recorder-card">
                ...
                <div className="vi-mic-section">
                    ...
                    <p className="vi-mic-status">
                        {loading
                            ? t('interviewMicStatusProcessing')
                            : voice.isRecording
                                ? t('interviewMicStatusRecording')
                                : voice.transcript
                                    ? t('interviewMicStatusSaved')
                                    : t('interviewMicStatusIdle')
                        }
                    </p>
                </div>
                ...
                {(displayText || voice.isRecording) && (
                    <div className="vi-transcript-box">
                        <span className="vi-transcript-label">
                            {voice.isRecording ? t('interviewTranscriptListening') : t('interviewTranscriptLabel')}
                        </span>
                        <p className="vi-transcript-text">
                            {displayText || <em className="vi-transcript-placeholder">{t('interviewTranscriptPlaceholder')}</em>}
                        </p>
                        ...
                        {!voice.isRecording && voice.transcript && (
                            <div className="vi-transcript-actions">
                                ...
                                >
                                    {loading
                                        ? <><Loader2 size={16} className="iv-spin" /><span>{t('analyzing')}</span></>
